...
class BiaffineModel(torch.nn.Module):
    def __init__(self, args):
        ...
        self.biaffine = Biaffine(args.biaffine_size, args.biaffine_size, 4)

        self.dropout_output = torch.nn.Dropout(0.1)

        self.ner_hidden_layer = torch.nn.Linear(args.bert_feature_dim, args.bert_feature_dim)
        self.ner_layer = torch.nn.Linear(args.bert_feature_dim,5)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        ner = self.ner_hidden_layer(bert_feature)
        ner = self.ner_layer(ner)

        return self.biaffine_forward(start,end,attention_mask), ner